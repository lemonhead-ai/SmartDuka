...
from src.contracts.shop import CatalogItemResponse, ShopResponse, ShopSetupRequest
...
async def get_shop(db: DatabaseSession) -> ShopResponse:
    ...
    return ShopResponse(id=shop.id, name=shop.name, category=shop.category, items=[CatalogItemResponse(id=item.id, name=item.name, category=item.category, price_kes=ShopInventoryManager.price(item), image_placeholder=item.image_placeholder) for _, item in stock])
...
async def create_shop(payload: ShopSetupRequest, db: DatabaseSession) -> ShopResponse:
    ...
    await db.commit()
    return ShopResponse(id=shop.id, name=shop.name, category=shop.category, items=[CatalogItemResponse(id=item.id, name=item.name, category=item.category, price_kes=ShopInventoryManager.price(item), image_placeholder=item.image_placeholder) for item in selected if item])
