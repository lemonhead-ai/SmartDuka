...
function MessageBubble({ side, children }: { side: "incoming" | "outgoing"; children: React.ReactNode }) {
  return (
    <motion.p
      ...
      className={`w-fit max-w-[90%] rounded-[18px] px-3 py-2 text-sm leading-relaxed ${side === "outgoing" ? "ml-auto bg-[#007AFF] text-white" : "bg-[#e5e5ea] text-[#1C1C1E] dark:bg-[#2C2C2E] dark:text-white"}`}
    >
      {children}
    </motion.p>
  );
...
function TypingIndicator() {
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex w-fit gap-1 rounded-[18px] bg-[#e5e5ea] dark:bg-[#2C2C2E] px-3 py-3" aria-label="Customer is typing">
      {[0, 1, 2].map((dot) => (
        <motion.span
          key={dot}
